...
#define EVENT_REALSE    (0x01<<0) // 释放事件
#define EVENT_PRESS     (0x01<<1) // 按下事件
#define EVENT_LONGPRESS (0x01<<2) // 长按事件
#define EVENT_BURSTS    (0x01<<3) // 连发事件
...
typedef struct{
    ...
    uint8_t read;
    uint8_t write;
}KEY_EVENT_FIFO_t;// 事件队列结构体

key_struct_t KEY_Struct[KEY_ID_MAX];// 按键结构体数组
KEY_EVENT_FIFO_t KEY_FIFO;// 事件队列
...
// 按键状态更新与事件触发保存
void decate_state(uint8_t key_id, KEY_Value_t curr_value){
        ...
        case KEY_PRESS:
            if(curr_value == PRESS){// 再次检测是否处于按下状态，以防止误操作
                if(KEY_Struct[key_id].allow_event & EVENT_PRESS){
                    EVENT_FIFO_Push((Key_evnet_t)(key_id*KEY_EVENTS_NUM + 1));
                }
                    ...
                    if(KEY_Struct[key_id].long_time_count < KEY_Struct[key_id].long_time_max){// 如果未达到长按触发时间，则继续计时
                        ...
                        if(KEY_Struct[key_id].long_time_count >= KEY_Struct[key_id].long_time_max){
                            if(KEY_Struct[key_id].allow_event & EVENT_LONGPRESS){// 如果允许触发长按事件
                                EVENT_FIFO_Push((Key_evnet_t)(key_id*KEY_EVENTS_NUM + 2));
                            }
                        }
                        ...
                        if(KEY_Struct[key_id].bursts_time_max > 0){
                            ...
                            if(KEY_Struct[key_id].bursts_time_max <= KEY_Struct[key_id].bursts_time_count){
                                if(KEY_Struct[key_id].allow_event & EVENT_BURSTS){// 如果允许触发连发事件
                                    EVENT_FIFO_Push((Key_evnet_t)(key_id*KEY_EVENTS_NUM + 3));   
                                }
        ...
        case KEY_REALSE:
            if(curr_value == REALSE){
                if(KEY_Struct[key_id].allow_event & EVENT_REALSE){// 如果允许触发松开事件
                    EVENT_FIFO_Push((Key_evnet_t)(key_id*KEY_EVENTS_NUM + 0));
                }
...
void Key_Scan(void) {
    ...
    for(i=0;i<KEY_ID_MAX;i++){
        ...
        decate_state((uint8_t)i,curr_value);
    }
}

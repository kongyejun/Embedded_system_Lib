...
void PWM_Init()
{
	...
	//配置OC1端口所需要的GPIO口
	GPIO_InitTypeDef GPIOA_Pin;
	GPIOA_Pin.GPIO_Mode=GPIO_Mode_AF_PP;
	GPIOA_Pin.GPIO_Pin=GPIO_Pin_2;
	GPIOA_Pin.GPIO_Speed=GPIO_Speed_50MHz;
	
	GPIO_Init(GPIOA,&GPIOA_Pin);
	...
	TIM2_OCInitSturct.TIM_OCPolarity=TIM_OCPolarity_High;//有效电平为高
	TIM2_OCInitSturct.TIM_OutputState=ENABLE;
	
	TIM_OC3Init(TIM2,&TIM2_OCInitSturct);

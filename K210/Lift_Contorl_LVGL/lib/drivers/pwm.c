...
double pwm_set_frequency(pwm_device_number_t pwm_number, pwm_channel_number_t channel, double frequency, double duty)
{
    ...
    int32_t periods = (int32_t)(clk_freq / frequency);//计算中断一次的计数值
    configASSERT(periods > 0 && periods <= INT32_MAX);
    frequency = clk_freq / (double)periods;           //计算保存真实值

    uint32_t percent = (uint32_t)(duty * periods);    //计算占空比所需值
    timer[pwm_number]->channel[channel].load_count = periods - percent;//计算其中一个电平周期的计数值
    timer[pwm_number]->load_count2[channel] = percent;//计算另一个电平周期的计数值
    return frequency;
    //K210通过两个计数寄存器交替运行实现缓冲PWM的实现，在修改占空比时可在运行时随意修改
}

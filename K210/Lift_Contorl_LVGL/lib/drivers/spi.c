...
typedef struct _my_spi_dma_context{
    plic_irq_callback_t callback;
} my_spi_dma_context;

my_spi_dma_context myspi_dma_context[4];
              ...
              size_t data_bit_length, uint32_t endian)
{
    ...
    // 根据frame_format，断言data_bit_length必须是偶数
    switch (frame_format)
    {
        case SPI_FF_DUAL:
    ...
    // 定义spi_adapter
    volatile spi_t *spi_adapter = spi[spi_num];
    // 如果baudr等于0，则将其设置为0x14
    if(spi_adapter->baudr == 0)
        spi_adapter->baudr = 0x14;
    // 将imr，dmacr，dmatdlr，dmardlr，ser，ssienr，ctrlr0，spi_ctrlr0，endian设置为0
    ...
    // 将work_mode，frame_format，data_bit_length设置到ctrlr0中
                              ...
                              const void *tx_buff, size_t tx_len, spi_transfer_width_t spi_transfer_width)
{
    ...
    while ((spi_handle->sr & 0x05) != 0x04)
        ;
    spi_handle->ser = 0x00;     //取消芯片选择
    spi_handle->ssienr = 0x00;  //禁用SPI设备
}

static uint32_t *buf=NULL;// 为了将16位数据转换为32位数据而申请的缓冲区指针，方便在dma中断中进行释放，避免内存泄漏
int spi_senddata_irq(void* ctx){
    configASSERT(ctx);
    my_spi_dma_context* spidma_context = (my_spi_dma_context*)ctx;
    if(spidma_context->callback){
        spidma_context->callback(NULL);
    }
    free((void *)buf);
    buf=NULL;
    return 0;
...
void spi_send_data_normal_dma_irq(dmac_channel_number_t channel_num, spi_device_num_t spi_num,
                                ...
                                spi_transfer_width_t spi_transfer_width,plic_irq_callback_t callback)
{
    configASSERT(spi_num < SPI_DEVICE_MAX && spi_num != 2);

    volatile spi_t *spi_handle = spi[spi_num];
    uint8_t tmod_offset = 0;int i;
    ...
    myspi_dma_context[spi_num].callback = callback;

    dmac_irq_register(channel_num,spi_senddata_irq,(void*)(&myspi_dma_context[spi_num]),5);
    spi_handle->ser = 1U << chip_select; 
}

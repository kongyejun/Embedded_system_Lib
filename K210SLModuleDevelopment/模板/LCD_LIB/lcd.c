...
// 按行划线
void lcd_draw_LinePoint(uint16_t x, uint32_t color){
    lcd_set_area(x, 0, x, LCD_Y_MAX);
    tft_fill_data(&color, LCD_Y_MAX);
}
// 按列划线
void lcd_draw_RowPoint(uint16_t y,uint16_t W,uint32_t color){
    lcd_set_area(0, y, W, y);
    tft_fill_data(&color, W);
}
...
/* 设置显示某个点的颜色 */
void lcd_draw_point_Tck(uint16_t x, uint16_t y, uint32_t color,uint8_t tickness){
    lcd_set_area(x-tickness, y-tickness, x+tickness, y+tickness);
    tft_fill_data(&color, (2*tickness+1)*(2*tickness+1));
}
...
/* 清除局部区域 */
void lcd_clear_part(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color){
    uint32_t data = ((uint32_t)color << 16) | (uint32_t)color;
    lcd_set_area(x1, y1, x2, y2);
    tft_fill_data(&data, (x2 - x1 + 1) * (y2 - y1 + 1) / 2);
}
/* LCD画方框 */
...
/*
 设置滚动区域
 start + mid + end 必须满足320行!!!!!!!!!!!!!
*/
int8_t lcd_VerticalScroll(uint16_t start,uint16_t mid,uint16_t end){
    if (start + mid + end != 320) {
        return -1;
    }
    tft_write_command(VERTICAL_SCROLL_DEFINE);
    uint8_t data[6];
    data[0] = start>>8;
    data[1] = start;
    data[2] = mid>>8;
    ...
    data[4] = end>>8;
    data[5] = end;
    tft_write_byte(data,6);
    return 0;
}

// 设置滚动显示起始地址
void lcd_VerticalScrollAdd(uint16_t* Add){
    tft_write_command(VERTICAL_SCROLL_S_ADD);
    tft_write_half(Add,1);
}
#include "ICM2060.h"
#include "I2C_bsp\I2C_CTL.h"
#include "sleep.h"
#include "LOG_SYSTEM/LOG.h"
#include "math.h"

#define G_LC(L)  16.4f * (2000 / (L))  //角速度精度
#define A_LC(L)  2048  * (16   / (L))  //加速度精度

#define PI 3.14f
#define G 9.80f         //m/s^2
...
static float Kp=10.5,Ki=0.0005,Kd = 0;//比例系数，积分系数
...
void ICM_GetValues(volatile ICM_ACCEL *Accel,volatile ICM_GYRO *Gyro,uint8_t LC_A, uint16_t LC_G) {
    ...
    Gyro->gx = (((float) Gyro->gx - GyroOffset[0]) * PI / 180.0f ) / G_LC(LC_G); 
    Gyro->gy = (((float) Gyro->gy - GyroOffset[1]) * PI / 180.0f ) / G_LC(LC_G);
    Gyro->gz = (((float) Gyro->gz - GyroOffset[2]) * PI / 180.0f ) / G_LC(LC_G);
}
...
ICM2060_STATE ICM_AHRSupdate(volatile ICM_ACCEL *Data,volatile ICM_GYRO *GYRO) {
    static float fex=0,fey=0,fez=0;
    ...
    ezInt += Ki * ez;
    // PID中的D项平滑
    float edx = (ex-fex)/(halfT*2)*Kd;
    float edy = (ey-fey)/(halfT*2)*Kd;
    float edz = (ez-fez)/(halfT*2)*Kd;
    fex = ex;
    fey = ey;
    fez = ez;
    // 使用PID控制器消除向量积误差(陀螺仪漂移误差)
    GYRO->gx += Kp * ex +  exInt + edx;
    GYRO->gy += Kp * ey +  eyInt + edy;
    GYRO->gz += Kp * ez +  ezInt + edz;
...
void Quaternion_To_EulerAngles() {
    ...
    ICM_EndData.roll = atan2f(2 * q2 * q3 + 2 * q0 * q1, q0*q0-q1*q1-q2*q2+q3*q3) * 180.0 / PI; 
...
ICM2060_STATE ICM2060_FULL_Init(){
    ...
    if(res == ICM2060_OK){
        ...
        res = ICM_Write(SMPLRT_DIV, 0x07);      //采样速率 SAMPLE_RATE = INTERNAL_SAMPLE_RATE / (1 + SMPLRT_DIV)
        res = ICM_Write(GYRO_CONFIG, 0x18);     //+—2000 dps
        res = ICM_Write(ACCEL_CONFIG, 0x08);    //±4g
...
// void Mean_ACCGYRO(ICM_ACCEL *Accel, ICM_GYRO *Gyro,uint8_t LC_A, uint16_t LC_G){
//     float alpha = 0.1; // 滤波系数
//     ICM2060_GeOriginalData();
//     // 单位g
//     Accel->ax = ((float) Accel->ax) / A_LC(LC_A);
//     Accel->ay = ((float) Accel->ay) / A_LC(LC_A);
//     Accel->az = ((float) Accel->az) / A_LC(LC_A);
//     Gyro->gx = ((float) Gyro->gx - GyroOffset[0]) * G_LC(LC_G);     
//     Gyro->gy = ((float) Gyro->gy - GyroOffset[1]) * G_LC(LC_G);
//     Gyro->gz = ((float) Gyro->gz - GyroOffset[2]) * G_LC(LC_G);
//     float pitch_accel = atan2(Accel->ax, sqrt(Accel->ay * Accel->ay + Accel->az * Accel->az));
//     float roll_accel = atan2(Accel->ay, sqrt(Accel->ax * Accel->ax + Accel->az * Accel->az));
//     ICM_EndData.pitch = alpha * (ICM_EndData.pitch + Gyro->gy * halfT*2) + (1 - alpha) * pitch_accel * 180 / PI;
//     ICM_EndData.roll  = alpha * (ICM_EndData.roll + Gyro->gx * halfT*2) + (1 - alpha) * roll_accel * 180 / PI;
// }
...
void ICM2060_PosSlove(){
    ICM2060_GeOriginalData();
    ICM_GetValues(&accel, &gyro , 4, 2000);

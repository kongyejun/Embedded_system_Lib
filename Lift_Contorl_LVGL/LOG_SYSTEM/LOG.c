...
void __EM_LOG(const int LEVEL, const char* fun, const int line, const char* fmt, ...){
    ...
    if (LEVEL >= LOG_LEVEL){
        ...
        va_end(arg);
        //输出 日志等级 信息位置
        printf("[%s] [\033[0m\033[1;33m%s\033[0m \033[0m\033[1;32m%d\033[0m] ", EM_LOGLevelGet(LEVEL), fun, line);
        //输出 日志信息
        switch (LEVEL){
            case 0: printf("%s", buff);break;
            case 1: printf("\033[32m%s\033[0m", buff);break;
            case 2: printf("\033[35m%s\033[0m", buff);break;
            case 3: printf("\033[31m%s\033[0m", buff);break;
        }
        free(buff);
        buff = NULL;
    }

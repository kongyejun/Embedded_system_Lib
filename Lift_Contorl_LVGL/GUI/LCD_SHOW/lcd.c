...
/* LCD搭配DMA中断进行画图片 */
void lcd_draw_picture_half_irq(uint16_t x1, uint16_t y1, uint16_t width, uint16_t height, uint16_t *ptr,
                                plic_irq_callback_t callback){
    lcd_set_area(x1, y1, x1 + width - 1, y1 + height - 1);
    tft_write_half_irq(ptr, width * height,callback);//注意图片数据此处为16位
}

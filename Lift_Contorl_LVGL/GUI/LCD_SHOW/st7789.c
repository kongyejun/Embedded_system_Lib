...
{
    gpiohs_set_drive_mode(LCD_RS_GPIOHSNUM, GPIO_DM_OUTPUT);
    gpiohs_set_pin(LCD_RS_GPIOHSNUM, GPIO_PV_HIGH);
}

/* 开始传输命令 */
static void set_start_cmd(void)
{
    gpiohs_set_pin(LCD_RS_GPIOHSNUM, GPIO_PV_LOW);
}
...
{
    gpiohs_set_drive_mode(LCD_RST_GPIOHSNUM, GPIO_DM_OUTPUT);
    gpiohs_set_pin(LCD_RST_GPIOHSNUM, GPIO_PV_HIGH);
}

/* 设置LCD-RST电平 */
static void set_rst(uint8_t val)
{
    gpiohs_set_pin(LCD_RST_GPIOHSNUM, val);
}
...
/* SPI写数据（uint16_t类型） */
void tft_write_half_irq(uint16_t *data_buf, uint32_t length,plic_irq_callback_t callback){
    set_start_data();
    spi_init(SPI_CHANNEL, SPI_WORK_MODE_0, SPI_FF_OCTAL, 16, 0);
    spi_init_non_standard(SPI_CHANNEL, 16 /*instrction length*/, 0 /*address length*/, 0 /*wait cycles*/,
                          SPI_AITM_AS_FRAME_FORMAT /*spi address trans mode*/);
    spi_send_data_normal_dma_irq(DMAC_CHANNEL0, SPI_CHANNEL, SPI_SLAVE_SELECT, data_buf, length, SPI_TRANS_SHORT,callback);
}
